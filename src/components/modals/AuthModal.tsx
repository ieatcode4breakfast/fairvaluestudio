...
export function AuthModal(props: AuthModalProps) {
  ...
  useModalHistory(props.showLoginModal, () => props.setShowLoginModal(false), 'AuthModal');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) { props.setShowLoginModal(false); props.setLoginError(''); props.setLoginEmail(''); props.setLoginPassword(''); } }}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6 animate-in fade-in zoom-in-95">
        ...
        {props.activeTab === 'login' ? (
          <>
            <h3 className="text-lg font-medium text-slate-900 dark:text-slate-200 mb-2">Log In</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Enter your credentials to access your valuations.</p>
            <div className="space-y-4 mb-6">
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors text-sm"
                autoFocus
                onKeyDown={(e) => {
          ...
          <>
            <h3 className="text-lg font-medium text-slate-900 dark:text-slate-200 mb-2">Sign Up</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Create a new account to get started.</p>
            <div className="space-y-4 mb-6">
              <input
                type="email"
                placeholder="Email"
