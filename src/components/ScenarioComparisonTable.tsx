...
export function ScenarioComparisonTable({ scenarios, allResults }: Props) {
  ...
  return (
    <div className="bg-white rounded-none md:rounded-2xl shadow-sm border-y border-x-0 md:border-x border-slate-100 overflow-hidden mt-5 mb-4">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full p-4 flex items-center justify-between bg-white hover:bg-slate-50 transition-colors"
      >
        <h2 className="text-lg font-medium text-slate-800">Compare Scenarios</h2>
        {isExpanded ? (
      ...
      {isExpanded && (
        <div className="p-4 pt-0 border-t border-slate-100 overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {scenarios.map((sc, i) => {
                ...
                const mosColor = mos > 0 ? 'text-emerald-600' : 'text-red-600';
                const upsideColor = upside > 0 ? 'text-emerald-600' : 'text-red-600';
                ...
                return (
                  <tr key={sc.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-3 text-sm font-medium text-slate-800 text-right whitespace-nowrap">
                      ${iv.toFixed(2)}
                    </td>
