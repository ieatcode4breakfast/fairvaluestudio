...
async function fetchTTMData(ticker, companyName) {
    ...
    PERFORM A FRESH WEB SEARCH for the most recent Trailing Twelve Months (TTM) financial data for:
    Company Name: ${companyName}
    Ticker: ${ticker}

    Return exactly these metrics in a flat JSON format with these exact keys:
    - "revenue" (TTM)
    - "freeCashFlow" (TTM)
    - "netIncome" (TTM)
    - "sharesOutstanding" (Current/Latest)
    - "currency" (e.g., "USD")

    Constraints:
    - Base your search on the latest available public financial filings as of ${currentDate}.
        ...
        const response = await fetch(OPENROUTER_API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    { 
                        role: 'system', 
                        content: 'You are a financial data extractor. You must ALWAYS return a single, valid JSON object with no prose. If exact data is missing, provide your best grounded estimate based on the latest available filings.' 
                    },
...
async function runTest() {
    try {
        const result = await fetchTTMData('GOOGL', 'Alphabet Inc.');
        console.log('\nFinal Results:');
